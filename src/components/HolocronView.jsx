...
const HolocronView = ({ onNavigate, onToggleSave, savedExcerpts, onOpenArticle }) => {
    ...
    return (
        <div className="min-h-screen bg-deep-space relative overflow-hidden flex flex-col font-sans">
            ...
            {/* Application Header */}
            <header className="relative z-20 p-4 md:p-6 backdrop-blur-md bg-black/20 border-b border-white/5">
                <div className="max-w-7xl mx-auto flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
                    {/* Left: Title */}
                    <div>
                        <h1 className="text-xl md:text-2xl font-cinzel tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-cyan-400">
                            AKASHA // HOLOCRON
                        </h1>
                        <p className="text-[10px] text-gray-500 font-mono uppercase tracking-widest mt-1">5 Spheres of Galactic Knowledge</p>
                    </div>

                    {/* Right: Navigation Hub - Stacked on Mobile */}
                    <nav className="flex flex-wrap gap-2 items-center">
                        <button
                            onClick={() => onNavigate('timeline')}
                            className="flex items-center gap-2 px-3 md:px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 hover:border-blue-500/30 transition-all group"
                            aria-label="View Timeline"
                        >
                            <Database className="w-4 h-4 text-blue-400 group-hover:text-blue-300" />
                            <span className="text-xs font-mono uppercase tracking-widest text-gray-400 group-hover:text-white hidden sm:inline">Timeline</span>
                        </button>
                        <button
                            onClick={() => onNavigate('civilizations')}
                            className="flex items-center gap-2 px-3 md:px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 hover:border-cyan-500/30 transition-all group"
                            aria-label="View Civilizations"
                        >
                            <Globe className="w-4 h-4 text-cyan-400 group-hover:text-cyan-300" />
                            <span className="text-xs font-mono uppercase tracking-widest text-gray-400 group-hover:text-white hidden sm:inline">Races</span>
                        </button>
                        <button
                            onClick={() => onNavigate('topics')}
                            className="flex items-center gap-2 px-3 md:px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 hover:border-green-500/30 transition-all group"
                            aria-label="View Neural Net"
                        >
                            <Activity className="w-4 h-4 text-green-400 group-hover:text-green-300" />
                            <span className="text-xs font-mono uppercase tracking-widest text-gray-400 group-hover:text-white hidden sm:inline">Neural Net</span>
                        </button>
                        <button
                            onClick={() => onNavigate('saved')}
                            className="flex items-center gap-2 px-3 md:px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 hover:border-yellow-500/30 transition-all group"
                            aria-label="View Saved"
                        >
                            <Star className="w-4 h-4 text-yellow-500/70 group-hover:text-yellow-400" />
                            <span className="text-xs font-mono uppercase tracking-widest text-gray-400 group-hover:text-white hidden sm:inline">Saved</span>
                        </button>
                        <button
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-3 md:px-4 py-2 rounded-full border border-purple-500/30 bg-purple-500/10 hover:bg-purple-500/20 hover:border-purple-400 transition-all group"
                            title="Oracle"
                            aria-label="Oracle"
                        >
                            <MessageSquare className="w-4 h-4 text-purple-300" />
                            <span className="text-xs font-mono uppercase tracking-widest text-purple-200 hidden md:inline">Oracle</span>
                        </button>
                    </nav>
                </div>
            </header>
